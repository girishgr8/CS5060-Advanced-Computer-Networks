...
#define ACCESSTOKEN "videoservertoken"
...
int main()
{
    ...
    bootSrv.sin_addr.s_addr = inet_addr("127.0.0.1");
    // sin_zero is a char[8] array where we will set all the values to zero
    memset(&(bootSrv.sin_zero), 0, 8);

    string fileserver = "servicename:VideoServer,servicetype:video,ipaddress:127.0.0.1,portnumber:" + to_string(PORT) + ",serviceaccesstoken:" + ACCESSTOKEN;
